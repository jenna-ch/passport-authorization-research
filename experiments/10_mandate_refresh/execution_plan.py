...
# execution plan for P3-D2.
#
# ADAPTED, NOT FROZEN. This is P3-B's execution_plan.py with four identifiers
# changed: the schema name, the cell name, the run_id prefix and the design-of-
# record pointer. The plan mechanics — one plan generated once from a recorded
# seed, positions as the authority for run identity, regeneration-and-digest
# verification, blocks-of-three interleaving, and resumption by position — are
# the ones P3-B, P3-B2 and P3-A all ran on, unchanged. The diff against the
# P3-B file is printed by an offline gate so the adaptation is visible.
#
...
PLAN_SCHEMA = "p3d2.execution_plan.v1"
...
def records_exist(out_dir):
    """any run record already written against this plan.

    The plan is the authority for run identity, so once a single record exists
    the plan can never be rewritten — `--write-plan` refuses on this, not only
    on the plan file's own existence.
    """
    if not out_dir.exists():
        return []
    return sorted(p.name for p in out_dir.glob("p3d2_*.json"))
...
def build_plan_document(seed, n_per_arm, frozen_rows, prompt_hashes, arm_defs):
    ...
    return {
        "schema": PLAN_SCHEMA,
        "cell": "P3-D2",
        "design_of_record": "phase3_p3d2_design_and_implementation.md",
        "order_seed": seed,
        "n_per_arm": n_per_arm,
        ...
        "run_id_rule": ("run_id = p3d2_{position:03d}_{arm}. positions are "
